...
input2(){
	...
	if [ $frst -eq $frst ] && [ $scnd -eq $scnd ] ; then
		if [ -z $frst ] || [ -z $scnd ] ; then
			# show error message when one value is null
			print_banner
			input_err
			input2
		elif [ -n $frst ] && [ -n $scnd ] ; then
			# for just passing the statement
			echo ...
		else
			# show error when happen unexpected
			print_banner
			input_err
			input2
		fi
	else
		# show eror when happen other possiblities
		print_banner
		input_err
		input2
	fi
